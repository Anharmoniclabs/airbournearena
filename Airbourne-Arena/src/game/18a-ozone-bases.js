...
/* The bases live above a real visible cloud datum, not in empty black space.
   The additive storm plate makes the black source background disappear while
   keeping the authored cloud volume readable from the flight deck. */
var ozoneCloudLayer=new THREE.Group();ozoneCloudLayer.name='ozone cloud ocean';scene.add(ozoneCloudLayer);
var ozoneCloudMat=new THREE.MeshBasicMaterial({map:stormCloudVfxSkin,color:0xb8d7ef,
  transparent:true,opacity:LOW ? .28 : .45,blending:THREE.AdditiveBlending,depthWrite:false,side:THREE.DoubleSide});
for(var cloudN=0;cloudN<(LOW?1:2);cloudN++){
  /* Overscan keeps the rectangular texture boundary outside every base camera;
     the second rotated sheet breaks the single-image repetition. */
  var ozoneCloud=new THREE.Mesh(new THREE.PlaneGeometry(16000,10500),ozoneCloudMat);
  ozoneCloud.rotation.x=-Math.PI/2;ozoneCloud.rotation.z=cloudN*2.17;
  ozoneCloud.position.set(cloudN?1100:-800,SKYBASE_ALT-760-cloudN*170,cloudN?-900:600);ozoneCloudLayer.add(ozoneCloud);
}
var skyBaseHosts={};
...
var SKYBASE_ASSETS={
  vanguard:[['assets/vanguard-ozone-base-v2.glb','assets/vanguard-ozone-base-v1.glb'],
    ['assets/vanguard-ozone-base-v2-lod1.glb','assets/vanguard-ozone-base-v1-lod1.glb']],
  tempest:[['assets/tempest-ozone-base-v2.glb','assets/tempest-ozone-base-v1.glb'],
    ['assets/tempest-ozone-base-v2-lod1.glb','assets/tempest-ozone-base-v1-lod1.glb']],
  inferno:[['assets/inferno-ozone-base-v2.glb','assets/inferno-ozone-base-v1.glb'],
    ['assets/inferno-ozone-base-v2-lod1.glb','assets/inferno-ozone-base-v1-lod1.glb']]
};
function calibrateSkyBase(root,faction){
  var shell={vanguard:0xd7e4eb,tempest:0x174642,inferno:0x17181c}[faction],
  ...
  root.traverse(function(o){
    ...
    if(name.indexOf('aerospace shell')>=0||name.indexOf('orbital armor')>=0){o.material.color.setHex(shell);o.material.roughness=.48;}
    else if(name.indexOf('structural frame')>=0||name.indexOf('structural shadow')>=0){o.material.color.setHex(0x080d12);o.material.roughness=.34;}
    ...
    else if(name.indexOf('recessed glazing')>=0){o.material.color.setHex(faction==='inferno'?0x250903:0x071923);o.material.emissive.setHex(accent);o.material.emissiveIntensity=.42;}
...
Object.keys(SKYBASE_ASSETS).forEach(function(faction){
  ...
  function add(urls,distance,index){
    index=index||0;var url=urls[index];
    makeGltfLoader().load(url,function(gltf){
      ...
      if(index+1<urls.length){console.warn(faction+' v2 base unavailable; loading fallback',url);add(urls,distance,index+1);}
      else console.error(faction+' ozone base failed to load',url,err);
    });
  }
  add(SKYBASE_ASSETS[faction][0],0,0);
  add(SKYBASE_ASSETS[faction][1],LOW?700:1500,0);
...
function skyDeckAt(x,z){
  for(var faction in SKYBASE_POS){
    ...
    var inside=faction==='tempest'?(dx*dx+dz*dz<330*330):
      (Math.abs(dx)<(faction==='inferno'?430:410)&&Math.abs(dz)<(faction==='inferno'?160:140));
